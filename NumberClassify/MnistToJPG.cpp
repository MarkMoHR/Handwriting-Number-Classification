...
int main() {
	...
	string filename_test_images = "ImageData/t10k-images.idx3-ubyte";
	...
	string filename_test_labels = "ImageData/t10k-labels.idx1-ubyte";
	...
	ofstream testimagelistOutput("ImageData/test_imagelist.txt");

	//save test images  
	for (int i = 0; i < 10; i++)
		count_digits[i] = 0;
	string save_test_images_path_base = "ImageData/TestData/";
	...
	for (unsigned int i = 0; i < vec_test_images.size(); i++) {
		...
		char imgNameBuffer[512];
		image_name = "/" + image_name + ".jpg";
		sprintf(imgNameBuffer, "%s%d%s", save_test_images_path_base.c_str(), number, image_name.c_str());
		string image_full_name = std::string(imgNameBuffer);
		cv::imwrite(image_full_name, vec_test_images[i]);
		
		sprintf(imgNameBuffer, "%d%s\n", number, image_name.c_str());
		testimagelistOutput << imgNameBuffer;

	}

	testimagelistOutput.close();

	
	//------------------------------- Train Data -------------------------------//

	//read MNIST image into OpenCV Mat vector  
	string filename_train_images = "ImageData/train-images.idx3-ubyte";
	int number_of_train_images = 60000;
	vector<cv::Mat> vec_train_images;
	read_Mnist(filename_train_images, vec_train_images);

	//read MNIST label into int vector  
	string filename_train_labels = "ImageData/train-labels.idx1-ubyte";
	...
	ofstream trainimagelistOutput("ImageData/train_imagelist.txt");

	//save train images  
	for (int i = 0; i < 10; i++)
		count_digits[i] = 0;
	string save_train_images_path = "ImageData/TrainData/";
	...
	for (size_t i = 0; i < vec_train_images.size(); i++) {
		...
		char imgNameBuffer[512];
		image_name = "/" + image_name + ".jpg";
		sprintf(imgNameBuffer, "%s%d%s", save_train_images_path.c_str(), number, image_name.c_str());
		string image_full_name = std::string(imgNameBuffer);
		cv::imwrite(image_full_name, vec_train_images[i]);

		sprintf(imgNameBuffer, "%d%s\n", number, image_name.c_str());
		trainimagelistOutput << imgNameBuffer;

	}
	
	trainimagelistOutput.close();
